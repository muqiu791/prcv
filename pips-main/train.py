...
def run_model(model, d, device, I=6, horz_flip=False, vert_flip=False, sw=None, is_train=True):
    ...
    rgbs = d['rgbs'].to(device).float() # B, S, C, H, W 输入图像的 RGB 数据
    occs = d['occs'].to(device).float() # B, S, 1, H, W 输入的遮挡信息
    masks = d['masks'].to(device).float() # B, S, 1, H, W 输入的掩码信息
    trajs_g = d['trajs'].to(device).float() # B, S, N, 2 轨迹数据
    vis_g = d['visibles'].to(device).float() # B, S, N 可见性数据
    valids = d['valids'].to(device).float() # B, S, N 有效性数据
